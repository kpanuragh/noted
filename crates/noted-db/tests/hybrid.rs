...
async fn an_exact_rare_term_is_found_by_the_lexical_arm() {
    ...
    let target =
        page_with(&pool, ws, "Errors", "the deploy failed with error ECONNREFUSED today", 0).await;
    ...
    for i in 0..5 {
        ...
        decoys.push(page_with(&pool, ws, &title, &text, 700).await);
    }
...
/// exists to prove). `page_with` decoys carrying a real embedding under model
/// `M` would land in the vector arm's top-100 candidate set too — a small test
...
async fn a_semantic_match_is_found_by_the_vector_arm() {
    let (pool, ws) = setup().await;
    let target =
        page_with(&pool, ws, "Planner", "the query planner keeps choosing a seq scan", 3).await;
...
/// two pages with an IDENTICAL embedding on the same axis, one under `M` and
/// one under a different model, and asserts only the `M` page is returned.
...
async fn hybrid_vector_arm_ignores_other_models() {
    ...
    let target = page_with(&pool, ws, "Target", "notes without the query term", 5).await;
    ...
    let hits = search::hybrid(&pool, ws, "irrelevant-query-term", &vec_at(5), M, 10).await.unwrap();
    assert!(
