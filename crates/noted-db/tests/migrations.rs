...
async fn migration_seeds_the_default_workspace() {
    ...
    assert!(
        exists,
        "the web app and e2e suite depend on the seeded default workspace"
    );
}
...
async fn pages_cascade_delete_to_children() {
    let pool = fresh_pool().await;
    let ws: uuid::Uuid =
        sqlx::query_scalar("INSERT INTO workspaces (name) VALUES ('t') RETURNING id")
            .fetch_one(&pool)
            .await
            .unwrap();
    ...
    let remaining: i64 = sqlx::query_scalar("SELECT count(*) FROM pages WHERE workspace_id = $1")
        .bind(ws)
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!(
        remaining, 0,
        "deleting a parent page must cascade to its children"
    );
}
